...
public:
	...
	void loadModel(std::ifstream& is, AlignedMemoryPool *mem = NULL) {
		...
		char_tanh_conv.load(is, mem);
		char_left_lstm.load(is, mem);
		char_right_lstm.load(is, mem);
		char_state_hidden.load(is, mem);

		word_lstm.load(is, mem);
		word_state_hidden.load(is, mem);

		action_lstm.load(is, mem);
		action_state_hidden.load(is, mem);

		app_state_represent.load(is, mem);
		sep_state_represent.load(is, mem);

		sep_score.load(is, mem);
		app_score.load(is, mem);
